...
const OrderSuccessPage = () => {
    ...
    const fetchOrder = async () => {
        try {
            const { data } = await api.get(`/api/orders/${id}`, { withCredentials: true });
            setOrder(data);
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-slate-200 pb-32">
            <div className="max-w-4xl mx-auto px-6 lg:px-8 py-16">
                            ...
                            <div className="bg-zinc-900 border border-white/5 p-8 md:p-10 rounded-[3rem] shadow-2xl space-y-8">
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-zinc-800/50 p-6 rounded-2xl space-y-2">
                                        ...
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{order.shippingAddress.city}, {order.shippingAddress.postalCode}</p>
                ...
                >
                    <Link to="/products" className="group h-16 px-10 bg-primary-500 text-zinc-950 font-black uppercase tracking-[0.3em] overflow-hidden rounded-2xl transition-all hover:bg-white flex items-center justify-center gap-4 text-xs">
                        Continue Shopping <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </Link>
                    <Link to="/" className="h-16 px-10 bg-zinc-900 border border-white/5 text-slate-500 font-black uppercase tracking-[0.3em] rounded-2xl hover:text-white hover:bg-zinc-800 transition-all flex items-center justify-center gap-4 text-xs">
