...
const useAuthStore = create((set) => ({
    ...
    login: async (email, password) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined') localStorage.setItem('userInfo', JSON.stringify(data));
            return data;
        } catch (error) {
            set({ 
    ...
    updateProfile: async (profileData) => {
        ...
        try {
            ...
            const currentInfo = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('userInfo') || '{}') : {};
            const merged = { ...currentInfo, ...data };
            set({ userInfo: merged, isLoading: false });
            if (typeof window !== 'undefined') localStorage.setItem('userInfo', JSON.stringify(merged));
            return data;
        } catch (error) {
