        ...
        <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-[9px] font-black text-primary-600 uppercase tracking-[0.4em]">Order Details</span>
                    </div>
                    <h1 className="text-3xl md:text-5xl font-black text-zinc-950 italic tracking-tighter uppercase leading-none truncate">
                        ORDER: <span className="text-primary-600">{order._id.substring(0, 16)}...</span>
                    </h1>
                </div>
                <div className="flex items-center gap-3">
                    <button onClick={() => window.print()} className="px-6 py-3.5 bg-white border border-slate-200 rounded-xl text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-zinc-950 hover:border-slate-300 transition-all flex items-center gap-3 cursor-pointer shadow-sm">
                        <Printer className="w-4 h-4" /> Print Order
                    </button>
                    <div className={`px-6 py-3.5 rounded-xl text-[9px] font-black uppercase tracking-widest border shadow-sm ${order.isPaid ? 'bg-primary-50 border-primary-100 text-primary-600' : 'bg-red-50 border-red-100 text-red-600'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <div className="bg-white p-8 md:p-10 rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden">
                        <div className="flex items-center justify-between mb-10">
                            <h2 className="text-xl md:text-2xl font-black text-zinc-950 italic uppercase tracking-tighter flex items-center gap-4">
                                <Package className="w-6 h-6 text-primary-600" /> Order Items
                            </h2>
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{order.orderItems.length} ITEMS</span>
                        </div>
                            ...
                            {order.orderItems.map((item, idx) => (
                                <div key={idx} className="flex items-center gap-6 p-4 bg-slate-50 border border-slate-100 rounded-2xl group transition-all hover:border-primary-500/20">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-black text-zinc-950 uppercase italic truncate mb-1">{item.name}</p>
                                        <div className="flex items-center gap-4">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Qty: {item.qty}</span>
                                            <span className="text-[10px] font-black text-primary-600/70 uppercase tracking-widest">PKR {item.price.toLocaleString()}</span>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm font-black text-zinc-950 italic tracking-tighter">PKR {(item.qty * item.price).toLocaleString()}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h3 className="text-lg font-black text-zinc-950 uppercase italic tracking-tighter">Customer Details</h3>
                            </div>
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h3 className="text-lg font-black text-zinc-950 uppercase italic tracking-tighter">Shipping Address</h3>
                            </div>
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-primary-500 p-1 rounded-[2.5rem] shadow-sm relative overflow-hidden">
                        <div className="bg-white rounded-[2.4rem] p-8 space-y-6">
                            <div className="flex items-center justify-between">
                                <h3 className="text-xl font-black text-zinc-950 italic uppercase tracking-tighter">Payment Summary</h3>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="pt-4 border-t border-slate-100 flex justify-between items-end">
                                    <div>
                                        <span className="text-[9px] font-black text-primary-600 uppercase tracking-widest">Total Amount</span>
                                        <p className="text-3xl font-black text-zinc-950 italic tracking-tighter">PKR {order.totalPrice?.toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>

                            <div className="pt-4">
                                <div className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${order.isPaid ? 'bg-primary-500 text-zinc-950' : 'bg-red-500 text-white'}`}>
                    ...
                    {/* Operational Log */}
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                        <h3 className="text-lg font-black text-zinc-950 uppercase italic tracking-tighter mb-8 flex items-center gap-4">
