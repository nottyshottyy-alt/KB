...
const OrdersTab = ({ orders, loading }) => {
    ...
    if (orders.length === 0) return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-24 bg-zinc-900/50 rounded-[3rem] border border-white/5">
            ...
            <Link href="/products" className="inline-flex mt-8 text-primary-500 text-[10px] font-black uppercase tracking-widest hover:text-white transition-all">Shop Online →</Link>
        </motion.div>
    );
            ...
            {[...orders].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(order => (
                ...
                >
                    ...
                    <div className="pt-6 border-t border-white/5 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                        ...
                        <Link href={`/success/${order._id}`} className="group h-12 px-6 bg-zinc-950 border border-white/5 text-[9px] font-black text-white uppercase tracking-widest rounded-xl hover:bg-white hover:text-zinc-950 transition-all flex items-center justify-center gap-3">
...
const WishlistTab = ({ userInfo }) => {
    ...
    if (items.length === 0) return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-24 bg-zinc-900/50 rounded-[3rem] border border-white/5">
            ...
            <Link href="/products" className="inline-flex mt-8 text-primary-500 text-[10px] font-black uppercase tracking-widest hover:text-white transition-all">Go Shopping →</Link>
        </motion.div>
    );
    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {items.map(product => (
                ...
                >
                    <div className="block relative h-40 sm:h-48 bg-zinc-950 overflow-hidden group">
                    ...
                    <div className="p-3 sm:p-6 space-y-2 sm:space-y-4">
                        ...
                        <div className="flex items-center justify-between pt-1 sm:pt-2">
                            ...
                            <Link href={`/product/${product._id}`} className="p-1 sm:p-2 text-white/20 hover:text-primary-500 transition-colors">
...
const ProfileContent = () => {
    ...
    useEffect(() => {
        ...
        if (!userInfo) {
            ...
            const redirectUrl = currentTab ? `/login?redirect=/profile?tab=${currentTab}` : '/login?redirect=/profile';
                    ...
                    <nav className="lg:col-span-3 space-y-4">
                        ...
                        {TABS.map(tab => {
                            ...
                            return (
                                <button
                                    key={tab.id}
                                    onClick={() => { setActiveTab(tab.id); router.push(`/profile?tab=${tab.id}`); }}
